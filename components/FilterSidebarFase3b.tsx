...
import type { FiltersFase3B } from './DataTableFase3b'

interface Props {
  filters: FiltersFase3B
  onChange: (f: FiltersFase3B) => void
  onReset: () => void
  collapsed: boolean
...
const SORT_OPTIONS = [
  { value: 'puntuacion3b', label: 'Puntuación 3B' },
  { value: 'nombre', label: 'Nombre' },
  { value: 'id', label: 'Identificador' },
  { value: 'resultado3b', label: 'Resultado 3B' },
]

export const DEFAULT_FILTERS_FASE3B: FiltersFase3B = {
  search: '',
  resultado: [],
  scoreMin: 0,
  scoreMax: 120,
...
export function FilterSidebarFase3b({ filters, onChange, onReset, collapsed, onToggleCollapse }: Props) {
  const update = (partial: Partial<FiltersFase3B>) => onChange({ ...filters, ...partial })
