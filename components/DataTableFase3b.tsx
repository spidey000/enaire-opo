...
import { CandidatoFase3BOnly, ResultadoFase3 } from '@/lib/parseCSV'
import { ArrowUpDown, ArrowUp, ArrowDown, Search, X, Eye, List } from 'lucide-react'

export interface FiltersFase3B {
  search: string
  resultado: string[]
  scoreMin: number
  scoreMax: number
...
interface Props {
  data: CandidatoFase3BOnly[]
  filters: FiltersFase3B
...
export const COLUMNS_FASE3B = [
  { key: 'ranking', label: '#', align: 'text-right' },
  { key: 'id', label: 'Identificador' },
  { key: 'nombre', label: 'Nombre y Apellidos' },
  { key: 'resultado3b', label: 'Resultado 3B' },
  { key: 'puntuacion3b', label: 'Puntuación 3B', align: 'text-right' },
]

export function DataTableFase3b({ data, filters, onSortChange }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((c) => {
      ...
      if (c.puntuacion3b !== null) {
        if (c.puntuacion3b < filters.scoreMin || c.puntuacion3b > filters.scoreMax) return false
      } else if (filters.scoreMin > 0 || filters.scoreMax < 120) {
        return false
      }
      return true
    })
  ...
  return (
    <div className="bg-card border border-border rounded-sm shadow-sm overflow-hidden">
      ...
      <div className="w-full overflow-x-auto" aria-label="Tabla de resultados Fase 3b">
        <table className="w-full min-w-0 lg:min-w-[600px] text-xs table-auto md:table-fixed border-separate border-spacing-0">
          ...
          <tbody className="divide-y divide-border/60">
            {currentRows.length === 0 ? (
              <tr>
                <td colSpan={COLUMNS_FASE3B.length + 1} className="px-4 py-16 text-center text-sm text-muted-foreground">
              ...
              currentRows.map((c, i) => {
                ...
                >
                  ...
                  <td className="px-3 py-2.5"><ResultadoBadge resultado={c.resultado3b} /></td>
                  <td className="px-3 py-2.5 font-mono text-right font-bold text-[12px]">
                    {c.puntuacion3b !== null
                      ? <span className="text-foreground">{fmt(c.puntuacion3b)}</span>
      ...
      {unpinnedRows.length > 0 && !showAll && (
        <div className="px-4 py-3 border-t border-border bg-muted/20 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            ...
            >
              <label htmlFor="fase3b-page-input" className="text-xs text-muted-foreground">Ir a</label>
              <input
                id="fase3b-page-input"
                type="number"
                min={1}
                max={totalPages}
                value={pageInput}
