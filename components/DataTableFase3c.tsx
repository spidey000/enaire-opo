...
import { CandidatoFase3COnly, ResultadoFase3 } from '@/lib/parseCSV'
import { ArrowUpDown, ArrowUp, ArrowDown, Search, X, Eye, List } from 'lucide-react'

export interface FiltersFase3C {
  search: string
  resultado: string[]
  sortBy: string
  sortDir: 'asc' | 'desc'
...
export const COLUMNS_FASE3C = [
  { key: 'ranking', label: '#', align: 'text-right' },
  { key: 'id', label: 'Identificador' },
  { key: 'nombre', label: 'Nombre y Apellidos' },
  { key: 'resultado3c', label: 'Resultado 3C' },
]

export function DataTableFase3c({ data, filters, onSortChange }: Props) {
  ...
  return (
    <div className="bg-card border border-border rounded-sm shadow-sm overflow-hidden">
      ...
      <div className="w-full overflow-x-auto" aria-label="Tabla de resultados Fase 3C">
        <table className="w-full min-w-0 lg:min-w-[500px] text-xs table-auto md:table-fixed border-separate border-spacing-0">
          <colgroup>
            <col style={{ width: '8%' }} />
            <col style={{ width: '14%' }} />
            <col style={{ width: '40%' }} />
          ...
          <tbody className="divide-y divide-border/60">
            {currentRows.length === 0 ? (
              <tr>
                <td colSpan={COLUMNS_FASE3C.length + 1} className="px-4 py-16 text-center text-sm text-muted-foreground">
              ...
              currentRows.map((c, i) => {
                ...
                >
                  ...
                  <td className="px-3 py-2.5"><ResultadoBadge resultado={c.resultado3c} /></td>
                </tr>
              )})
            )}
          </tbody>
      ...
      {unpinnedRows.length > 0 && !showAll && (
        <div className="px-4 py-3 border-t border-border bg-muted/20 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            ...
            >
              <label htmlFor="fase3c-page-input" className="text-xs text-muted-foreground">Ir a</label>
              <input
                id="fase3c-page-input"
                type="number"
                min={1}
                max={totalPages}
                value={pageInput}
