...
import { CandidatoFase3BOnly } from '@/lib/parseCSV'
...
export function ChartsPanelFase3b({ data }: Props) {
  const resultadoData = useMemo(() => {
    const counts: Record<string, number> = {}
    for (const c of data) {
      const key = c.resultado3b ?? 'SIN DATO'
  ...
  const scores = useMemo(() => {
    return data
      .filter((c) => c.puntuacion3b !== null)
      .map((c) => c.puntuacion3b! as number)
  }, [data])

  const { buckets } = useMemo(
    () => buildScoreBuckets(scores, 10, 0),
    [scores]
  )
  ...
  const commonStats = useMemo(() => {
    ...
    const aptos = data.filter((c) => c.resultado3b === 'APTO/A').length
    const noAptos = data.filter((c) => c.resultado3b === 'NO APTO/A').length
    const excluidos = data.filter(
      (c) => c.resultado3b && c.resultado3b.startsWith('EXCLUIDO')
    ).length
    const renuncias = data.filter(
      (c) => c.resultado3b && c.resultado3b.startsWith('RENUNCIA')
    ).length
    const conPuntuacion = data.filter((c) => c.puntuacion3b !== null)
  ...
  // ─── Donut: Distribución por Resultado 3B ─────────────────────
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <ChartCard title="Distribución por Resultado 3B" sub="Resultado provisional Fase 3B">
          <Doughnut data={donutData} options={donutOptions} />
        </ChartCard>

        <ChartCard title="Distribución de Puntuación 3B (sin agrupar)" sub="Candidatos con puntuación 3B — cada punto entero">
          <Bar data={ungroupedBarData} options={ungroupedBarOptions} />
        </ChartCard>
      </div>

      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <ChartCard title="Distribución por tramos" sub="Puntuación Fase 3B — agrupado cada 10 puntos, máx. 120">
